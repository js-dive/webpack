...
To fix this, make sure to include [runtime] in the output.hotUpdateMainFilename option, or use the default config.`)
		...
		if (compiler.options.output.clean) {
			const staleHmrFiles = new Set();
			const emitHmrFiles = (() => {
				const maxSize = STALE_HMR_UPDATES + 1;
				const updates = [];
				return files => {
					if (updates.length === maxSize) {
						const stale = updates[0];
						for (let i = maxSize - 1; i > 0; i--) updates[i - 1] = updates[i];
						updates[maxSize - 1] = files;
						for (const file of stale) staleHmrFiles.delete(file);
					} else {
						updates.push(files);
					}
					for (const file of files) staleHmrFiles.add(file);
				};
			})();
			compiler.hooks.emit.tapAsync(
				...
				(compilation, callback) => {
					...
					for (const asset of Object.keys(compilation.assets)) {
						const { hotModuleReplacement } = compilation.assetsInfo.get(asset);
						if (hotModuleReplacement) files.push(asset);
					}
					emitHmrFiles(files);
					callback();
				}
			...
			compiler.hooks.compilation.tap(
				"HotModuleReplacementPlugin-clean-step",
				compilation => {
					const hooks = CleanPlugin.getCompilationHooks(compilation);
					hooks.keep.tap("HotModuleReplacementPlugin", file => {
						if (staleHmrFiles.has(file)) return true;
					});
				}
			);
		}
